...
template 'Settings Flytech' do
  path 'C:\chef\AzureUploader\settings.config'
  source 'settings-log_flytech.erb'
  variables({
    :node_name => Chef.run_context.node.name
  })
end

ruby_block 'AzureUploader Flytech (PowerShell)' do
  block do
...
ruby_block 'Delete .zip File Flytech' do
  block do
    Dir.foreach("C:\\") do |zipFile|
      next if !zipFile.start_with? Chef.run_context.node.name
      File.delete "C:\\#{zipFile}" if zipFile.end_with? '.zip'
    end
  end
